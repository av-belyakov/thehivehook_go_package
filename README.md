...
TheHiveHook_Go_Package является посредником между TheHive и NATS, и применяется для передачи событий типа 'case' и 'alert' TheHive через брокер сообщений NATS различным заинтересованным системам.
