FROM golang:1.23.4-alpine AS temporary_image_commit
WORKDIR /go/src/
...
COPY --from=temporary_image_commit /go/src/thehivehook_go_package /opt/thehivehook_go_package/ 
COPY --from=temporary_image_commit /go/src/README.md /opt/thehivehook_go_package/
